...
const CourseManagement = (props) => {
  ...
  const { data: coursewareList, loading, error, request: getCourseware } = useResource();
  ...
  const searchHandler = (imParam = {}) => {
    const params = {
      name: coursewareName ? coursewareName : undefined,
  ...
  const resetParams = () => setCoursewareName('');

  const handleCreate = () => {
    console.log(DrawerRef, 'DrawerRef')
    DrawerRef.current.handleShowDrawer()
  }

  const columns = [
    { title: '课程名称', dataIndex: 'name' },
    { title: '课程教师', dataIndex: 'format' },
    { title: '章节数量', dataIndex: 'size' },
      ...
      <Grid item xs={9}>
        ...
        <Stack direction="row" justifyContent="space-between" mb={2}>
          <Stack direction="row" spacing={2}>
            <RyButton sx={{ height: '100%' }} variant="contained" onClick={() => handleCreate()}>
              创建课程
            </RyButton>
          </Stack>
          <Stack direction="row" spacing={2}>
            <RyInput
              ...
              onChange={(evt) => setCoursewareName(evt.target.value)}
            />
